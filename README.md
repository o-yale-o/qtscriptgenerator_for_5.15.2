...
Qt 5.15 头文件 ──────────────┐
                            ▼
  typesystem_*.xml ──►  generator.exe  ──►  旁路产物: jsx/qt/*.jsx (IDL 声明，见下)
...
- **真面目**：生成器顺带输出的"绑定面 IDL 声明"，用 ECMAScript 风格的伪
  语言描述每个类暴露给脚本的接口（`native class QBasicTimer { function
  start(msec:int, obj:QObject):void }`），外加每模块一张 import 清单
  （`qt_core.jsx`）。文件头自带 *"generated by JSXgenerator, DO NOT EDIT"*。
- **历史用途**：Qt Labs 原项目里给 QtScript 的 ES4 前端工具当输入。
- **在本仓库**：既不编译进 dll，也不在运行时求值——它是"脚本世界长什么
  样"的机器可读快照/文档，与 `doc/` 同级性质。
