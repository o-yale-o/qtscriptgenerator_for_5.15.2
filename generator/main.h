...
struct Preprocess
{
	    // Added at 2025-04-03 by Lusp
	    static QString findResourceFile(const QString &relativePath) {
        // 1. 尝试当前目录
        QString appDir = QCoreApplication::applicationDirPath();
        QString localPath = appDir + "/" + relativePath;
        if (QFile::exists(localPath)) {
            return localPath;
        }
        
        // 2. 尝试上级目录的data文件夹
        QString dataPath = appDir + "/../data/" + relativePath;
        if (QFile::exists(dataPath)) {
            return dataPath;
        }
        
        // 3. 返回原始资源路径作为最后尝试
        return ":/trolltech/generator/" + relativePath;
    }
    ...
    static bool preprocess(const QString &sourceFile, const QString &targetFile, const QString &commandLineIncludes = QString())
    {
        ...
        */     
			  QString ppconfigPath = findResourceFile("parser/rpp/pp-qt-configuration");
        QFile file(ppconfigPath);
        if (!file.open(QFile::ReadOnly)) {
            fprintf(stderr, "Preprocessor configuration file not found at '%s'\n", 
                   qPrintable(ppconfigPath));
            return false;
        }        
        // end 2025-04-03
