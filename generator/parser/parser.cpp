  ...
  if (tk == '=')
    {
      ...
      // C++11 function definitions: "= default" / "= delete"
      // Also accept the classic pure-virtual "= 0".
      // These appear inside class bodies (e.g. QObjectData() = default;)
      // and the legacy parser below cannot turn the keywords into an
      // expression, which corrupts the token stream and makes the
      // parser skip the very next class declaration (e.g. QObject itself).
      int nextTk = token_stream.lookAhead();
